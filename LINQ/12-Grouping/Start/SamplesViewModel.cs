  ...
  public class SamplesViewModel : ViewModelBase
  {
    ...
    public List<IGrouping<string, Product>> GroupByQuery()
    {
      ...
      List<Product> products = ProductRepository.GetAll();
            ...
            list = (from prod in products orderby prod.Size group prod by prod.Size).ToList();
    ...
    public List<IGrouping<string, Product>> GroupByUsingKeyMethod()
    {
      ...
      List<Product> products = ProductRepository.GetAll();
            ...
            // Write Method Syntax Here
            list = products.GroupBy(prod => prod.Size).OrderBy(sizes => sizes.Key).ToList();
    ...
    public List<IGrouping<string, Product>> GroupByWhereMethod()
    {
      ...
      List<Product> products = ProductRepository.GetAll();
            ...
            // Write Method Syntax Here
            list = products.OrderBy(prod => prod.Size).GroupBy(products => products.Size).
                      Where(KeySizes => KeySizes.Count() > 2)
    ...
    public List<SaleProducts> GroupBySubQueryQuery()
    {
      ...
      List<SalesOrder> sales = SalesOrderRepository.GetAll();
        ...
        public List<SaleProducts> GroupBySubQueryMethod()
        {
            ...
            stopwatch.Start();
            //list = sales.OrderBy(s => s.SalesOrderID).GroupBy(sale => sale.SalesOrderID)
            //    .Select(newSales =>
            //    {
            //        var list = newSales.ToList();
            //        List<Product> productsRelatedToSale = new();
            //        foreach (var item in list)
            //        {
            //            productsRelatedToSale.AddRange(products.Where(p => p.ProductID == item.ProductID));
            //        }
            //        return new SaleProducts
            //        {
            //            SalesOrderID = newSales.Key,
            //            Products = productsRelatedToSale
    ...
    public List<string> GroupByDistinctMethod()
    {
      ...
      List<Product> products = ProductRepository.GetAll();
            ...
            // Write Method Syntax Here
            list = products.GroupBy(p => p.Color).Select(groupedColors => groupedColors.FirstOrDefault().Color)
                      .OrderBy(c => c).ToList();
