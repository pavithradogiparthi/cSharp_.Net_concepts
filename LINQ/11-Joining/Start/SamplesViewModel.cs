    ...
    public List<ProductOrder> InnerJoinTwoFieldsQuery()
    {
      ...
      List<SalesOrder> sales = SalesOrderRepository.GetAll();
            ...
            list = (from prod in products
                  ...
                  new { prod.ProductID, Qty = (short)6 } equals
    ...
    public List<ProductSales> JoinIntoQuery()
    {
      ...
      List<SalesOrder> sales = SalesOrderRepository.GetAll();
            ...
            list = (from prod in products
                    ...
                    join sale in sales
                  on prod.ProductID equals sale.ProductID into newSales
    ...
    public List<ProductSales> JoinIntoMethod()
    {
      ...
      List<SalesOrder> sales = SalesOrderRepository.GetAll();
            ...
            list = products.OrderBy(p => p.ProductID).GroupJoin(sales,
                prod => prod.ProductID,
                sale => sale.SalesOrderID, (prod, newSales) => new ProductSales
                {
                    Product = prod,
                    Sales = newSales.OrderBy(s => s.SalesOrderID).ToList()
    ...
    public List<ProductOrder> LeftOuterJoinQuery()
    {
      ...
      List<SalesOrder> sales = SalesOrderRepository.GetAll();
            ...
            list = (from prod in products join sale in sales on prod.ProductID equals sale.ProductID
                  into newSales from sale in newSales.DefaultIfEmpty()
                    select new ProductOrder {
                        ProductID = prod.ProductID,
                        Name = prod.Name,
                        Color = prod.Color,
...
list=products.SelectMany(prod=>sales.Where(s=>s.ProductID==prod.ProductID).DefaultIfEmpty(),
                (prod, sale) => new ProductOrder {
                    ProductID = prod.ProductID,
                    Name = prod.Name,
                    Color = prod.Color,
